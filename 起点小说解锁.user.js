...
(function () {
    ...
    function init() {
        ...
        if (!QDisVIP()) {
            ...
            maintail = document.querySelector('.vip-limit-wrap');
        }
        if (maintail == null) {
            alert('初始化失败')
    ...
    function QDgetBookChapter(doc) {
        let head = doc.querySelector(".j_chapterName > span.content-wrap");
        if (head) {
    ...
    function firstcontent(response) {
        ...
        let mainspace = document.querySelector('div > div.main-text-wrap');
        if (mainspace) {
            let read_content = mainspace.querySelector('.read-content');
            if (read_content) {
                mainspace.removeChild(read_content);
            }
            let h3 = maintail.querySelector('h3');
            if (h3) {
                h3.innerText = '已订阅本章章节';
            }
            let div = maintail.querySelector('div');
            if (div) {
                maintail.removeChild(div);
            }
            let p = maintail.querySelector('p');
            if (p) {
                maintail.removeChild(p);
            }
        }
